...
///         governed by the TrevorGames council.
...
contract TrevorMarketplace is AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20;

    struct ListingOrBid {
        /// @dev number of tokens for sale or requested (1 if ERC-721 token is active for sale) (for bids, quantity for
        /// ERC-721 can be greater than 1)
        uint64 quantity;
        /// @dev price per token sold, i.e. extended sale price equals this times quantity purchased. For bids, price
        /// offered per item.
    ...
    /// @notice TREVOR_MARKETPLACE_ADMIN_ROLE role hash
    bytes32 public constant TREVOR_MARKETPLACE_ADMIN_ROLE = keccak256("TREVOR_MARKETPLACE_ADMIN_ROLE");
    ...
    /// @notice the default token that is used for marketplace sales and fee payments. Can be overridden by
    /// collectionToTokenAddress.
    IERC20 public paymentToken;

    /// @notice fee portion (in basis points) for each sale, (e.g. a value of 100 is 100/10000 = 1%). This is the fee if
    /// no collection owner fee is set.
    uint256 public fee;
    ...
    /// @notice fee portion (in basis points) for each sale. This is used if a separate fee has been set for the
    /// collection owner.
    uint256 public feeWithCollectionOwner;

    /// @notice Maps the collection address to the fees which the collection owner collects. Some collections may not
    /// have a seperate fee, such as those owned by the TrevorGames.
    mapping(address => CollectionOwnerFee) public collectionToCollectionOwnerFee;

    /// @notice Maps the collection address to the payment token that will be used for purchasing. If the address is the
    /// zero address, it will use the default paymentToken.
    ...
    /// @param  _recipient   The recipient of the fees. If the address is 0, the collection fees for this collection
    /// have been removed.
    ...
    function initialize(
        ...
        initializer
    {
        require(address(_initialPaymentToken) != address(0), "TrevorMarketplace: cannot set address(0)");

        __AccessControl_init_unchained();
        __Pausable_init_unchained();
        __ReentrancyGuard_init_unchained();

        _setRoleAdmin(TREVOR_MARKETPLACE_ADMIN_ROLE, TREVOR_MARKETPLACE_ADMIN_ROLE);
        _grantRole(TREVOR_MARKETPLACE_ADMIN_ROLE, msg.sender);
    ...
    function createListing(
        ...
        whenNotPaused
    {
        require(listings[_nftAddress][_tokenId][_msgSender()].quantity == 0, "TrevorMarketplace: already listed");
    ...
    function updateListing(
        ...
        whenNotPaused
    {
        require(listings[_nftAddress][_tokenId][_msgSender()].quantity > 0, "TrevorMarketplace: not listed item");
    ...
    function _createListingWithoutEvent(
        ...
        internal
    {
        require(_expirationTime > block.timestamp, "TrevorMarketplace: invalid expiration time");
        require(_pricePerItem >= MIN_PRICE, "TrevorMarketplace: below min price");

        if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            IERC721 nft = IERC721(_nftAddress);
            require(nft.ownerOf(_tokenId) == _msgSender(), "TrevorMarketplace: not owning item");
            require(nft.isApprovedForAll(_msgSender(), address(this)), "TrevorMarketplace: item not approved");
            require(_quantity == 1, "TrevorMarketplace: cannot list multiple ERC721");
        } else if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_1155_APPROVED) {
            IERC1155 nft = IERC1155(_nftAddress);
            require(nft.balanceOf(_msgSender(), _tokenId) >= _quantity, "TrevorMarketplace: must hold enough nfts");
            require(nft.isApprovedForAll(_msgSender(), address(this)), "TrevorMarketplace: item not approved");
            require(_quantity > 0, "TrevorMarketplace: nothing to list");
        } else {
            revert("TrevorMarketplace: token is not approved for trading");
        }

        address _paymentTokenForCollection = getPaymentTokenForCollection(_nftAddress);
        require(_paymentTokenForCollection == _paymentToken, "TrevorMarketplace: Wrong payment token");
    ...
    {
        if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            require(_quantity == 1, "TrevorMarketplace: token bid quantity 1 for ERC721");
        } else if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_1155_APPROVED) {
            require(_quantity > 0, "TrevorMarketplace: bad quantity");
        } else {
            revert("TrevorMarketplace: token is not approved for trading");
        }
        ...
        whenBiddingActive
    {
        if (tokenApprovals[_nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            ...
            revert("TrevorMarketplace: No collection bids on 1155s");
        } else {
            revert("TrevorMarketplace: token is not approved for trading");
        }
        ...
        private
    {
        ...
        address _paymentTokenForCollection = getPaymentTokenForCollection(_nftAddress);
        require(_paymentTokenForCollection == _paymentToken, "TrevorMarketplace: Bad payment token");
        ...
        require(
            ...
            "TrevorMarketplace: Not enough tokens owned or allowed for bid"
        );
    ...
    function _acceptBid(AcceptBidParams calldata _acceptBidParams, BidType _bidType) private {
        // Validate buy order
        require(_msgSender() != _acceptBidParams.bidder, "TrevorMarketplace: Cannot supply own bid");
        require(_acceptBidParams.quantity > 0, "TrevorMarketplace: Nothing to supply to bidder");
        ...
        require(_bid.quantity > 0, "TrevorMarketplace: bid does not exist");
        require(_bid.expirationTime >= block.timestamp, "TrevorMarketplace: bid expired");
        require(_bid.pricePerItem > 0, "TrevorMarketplace: bid price invalid");
        require(_bid.quantity >= _acceptBidParams.quantity, "TrevorMarketplace: not enough quantity");
        require(_bid.pricePerItem == _acceptBidParams.pricePerItem, "TrevorMarketplace: price does not match");
        ...
        require(
            ...
            "TrevorMarketplace: Wrong payment token"
        );
        ...
        if (tokenApprovals[_acceptBidParams.nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            require(_acceptBidParams.quantity == 1, "TrevorMarketplace: Cannot supply multiple ERC721s");
    ...
    /// @notice Buy multiple listed items. You must authorize this marketplace with your payment token to completed the
    /// buy or purchase with eth if it is a weth collection.
    function buyItems(BuyItemParams[] calldata _buyItemParams) external payable nonReentrant whenNotPaused {
        ...
        require(msg.value == _ethAmountRequired, "TrevorMarketplace: Bad ETH value");
    }
    ...
    function _buyItem(BuyItemParams calldata _buyItemParams) private returns (uint256) {
        // Validate buy order
        require(_msgSender() != _buyItemParams.owner, "TrevorMarketplace: Cannot buy your own item");
        require(_buyItemParams.quantity > 0, "TrevorMarketplace: Nothing to buy");

        // Validate listing
        ListingOrBid memory listedItem =
            listings[_buyItemParams.nftAddress][_buyItemParams.tokenId][_buyItemParams.owner];
        require(listedItem.quantity > 0, "TrevorMarketplace: not listed item");
        require(listedItem.expirationTime >= block.timestamp, "TrevorMarketplace: listing expired");
        require(listedItem.pricePerItem > 0, "TrevorMarketplace: listing price invalid");
        require(listedItem.quantity >= _buyItemParams.quantity, "TrevorMarketplace: not enough quantity");
        require(listedItem.pricePerItem <= _buyItemParams.maxPricePerItem, "TrevorMarketplace: price increased");
        ...
        if (_buyItemParams.usingEth) {
            require(_paymentTokenForListing == address(weth), "TrevorMarketplace: ETH only used with weth collection");
        }

        // Transfer NFT to buyer, also validates owner owns it, and token is approved for trading
        if (tokenApprovals[_buyItemParams.nftAddress] == TokenApprovalStatus.ERC_721_APPROVED) {
            require(_buyItemParams.quantity == 1, "TrevorMarketplace: Cannot buy multiple ERC721");
    ...
    /// @dev pays the fees to the marketplace fee recipient, the collection recipient if one exists, and to the seller
    /// of the item.
        ...
        private
    {
        ...
        if (_usingEth) {
            ...
            require(_success, "TrevorMarketplace: Sending eth was not successful");
        } else {
    ...
    function getPaymentTokenForCollection(address _collection) public view returns (address) {
        ...
        // For backwards compatability. If a collection payment wasn't set at the collection level, it was using the
        // payment token.
    ...
    function _getPaymentTokenForListing(ListingOrBid memory listedItem) private view returns (address) {
        // For backwards compatability. If a listing has no payment token address, it was using the original, default
        // payment token.
    ...
    /// @notice Updates the fee amount which is collected during sales, for both collections with and without owner
    /// specific fees.
    ...
    function setFee(
        ...
        public
        onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE)
    {
        require(_newFee <= MAX_FEE && _newFeeWithCollectionOwner <= MAX_FEE, "TrevorMarketplace: max fee");
    ...
    /// @param _collectionOwnerFee The fee and recipient for the collection. If the 0 address is passed as the
    /// recipient, collection specific fees will not be collected.
        ...
        onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE)
    {
        require(
            ...
            "TrevorMarketplace: Collection is not approved"
        );
        require(_collectionOwnerFee.fee <= MAX_COLLECTION_FEE, "TrevorMarketplace: Collection fee too high");

        // The collection recipient can be the 0 address, meaning we will treat this as a collection with no collection
        // owner fee.
    ...
    function setFeeRecipient(address _newFeeRecipient) public onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE) {
        require(_newFeeRecipient != address(0), "TrevorMarketplace: cannot set 0x0 address");
        ...
        onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE)
    {
        if (_status == TokenApprovalStatus.ERC_721_APPROVED) {
            require(IERC165(_nft).supportsInterface(INTERFACE_ID_ERC721), "TrevorMarketplace: not an ERC721 contract");
        } else if (_status == TokenApprovalStatus.ERC_1155_APPROVED) {
            require(IERC165(_nft).supportsInterface(INTERFACE_ID_ERC1155), "TrevorMarketplace: not an ERC1155 contract");
        }

        require(
            _paymentToken != address(0) && (_paymentToken == address(weth) || _paymentToken == address(paymentToken)),
            "TrevorMarketplace: Payment token not supported"
        );
    ...
    function setWeth(address _wethAddress) external onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE) {
        require(address(weth) == address(0), "WETH address already set");

        weth = IERC20(_wethAddress);
    }

    function toggleAreBidsActive() external onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE) {
    ...
    function pause() external onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE) {
        _pause();
    }

    /// @notice Unpauses the marketplace, all functionality is restored
    /// @dev    This is callable only by the owner.
    function unpause() external onlyRole(TREVOR_MARKETPLACE_ADMIN_ROLE) {
        _unpause();
    }

    modifier whenBiddingActive() {
        require(areBidsActive, "TrevorMarketplace: Bidding is not active");
